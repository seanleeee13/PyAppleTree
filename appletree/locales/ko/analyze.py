translate_data = {
    ...
    "analyze_mfreport_2_3": "<orange><bold>재귀함수 - 자기 자신을 계속해서 호출하는 무거운 재귀함수일 가능성이 높습니다.<end>",
    "analyze_mfreport_3_1": "<yellow>일꾼 - 이 코드의 25~60% 정도의 부하가 되는 핵심 일꾼 함수입니다.<end>",
    "analyze_mfreport_3_2": "<yellow>잠재적 과부하 - 현재로써는 별다른 문제가 되지 않지만, 더 큰 데이터에서 문제가 될 수 있습니다.<end>",
    "analyze_mfreport_4_1": "<green>정상 - 별다른 부하를 주지 않는 정상적인 함수입니다. 데이터의 크기가 커지면 부하가 커질 수 있습니다.<end>",
    ...
    "analyze_lreport_recursion": "<orange><bold>재귀함수 - 재귀함수에서 자기자신을 호출하거나 큰 연산을 하는 곳으로 예상됩니다.<end>",
